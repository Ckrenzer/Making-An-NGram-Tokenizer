...
tokenizer_speeds_character <- microbenchmark::microbenchmark(
...
# Returns a tibble
tokenizer_speeds_tibble <- microbenchmark::microbenchmark(
...
tokenizer_speeds_character %>% 
  group_by(expr) %>% 
  summarize(median = round(median(time) / 10e8, 3)) %>% 
  knitr::kable()


tokenizer_speeds_tibble %>% 
  group_by(expr) %>% 
  summarize(median = round(median(time) / 10e8, 3)) %>% 
  knitr::kable()
