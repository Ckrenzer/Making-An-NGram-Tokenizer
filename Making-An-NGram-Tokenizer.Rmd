...
Data.table's `shift()` function provides us with a bona fide slider as described above. We can then use loop functions to assemble the n-grams. Do you have a shift list?
...
Of course, we might want to tokenize in a magrittr pipeline. When this is the case, character vectors usually won't cut it. Returning more complex data types allows for greater flexibility, but we pay for it at run time. To drive this point home, the three above functions are adjusted to return a tibble, and `f2_tibble_custom()` is a modified version of `f2()` that allows tokenization for mulitple documents, as well as control over the input column of text.
...
microbenchmark::microbenchmark(
  ...
  `Nested Loops Custom` = f2_tibble_custom(text_df = cristo, key_column = "doc_id", text_column = "text", n = 2),
  
  `tidytext` = tidytext::unnest_tokens(tbl = cristo, input = text, output = text, token = "ngrams", n = 2),
...
Depending on the day, `Heads & Tails` runs faster than the equivalent code in `tokenizers`. The function from `tokenizers` uses about half the memory of `Heads & Tails` and can be used with any value of *n*, however. But we did this entirely in R! We beat code written in C++ with a bit of clever writing.



Using a black box is much different from knowing how to use an algorithm, so hopefully you can apply the heuristics found here to truly understand what happens behind the scenes.