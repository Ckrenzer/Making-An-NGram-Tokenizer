...
Of course, we might want to use this in a magrittr pipeline. When this is the case, character vectors just won't cut it. Returning more complex data types allows for greater flexibility, but we pay for it at runtime. To drive this point home, the three above functions are adjusted to return a tibble, and `f2_tibble_custom()` is a modified version of `f2()` that allows tokenization for mulitple documents, as well as control over the input column of text.
...
To test the n-gram tokenizers, we need data! Let's use my favorite novel, [The Count of Monte Cristo](https://www.gutenberg.org/files/1184/1184-0.txt). The version used is the one from the `corpus` package's Project Gutenberg API, except the notes at the end of the book are removed. The data is stored in a text file to ensure reproducibility.
...
cristo_text <- str_c(cristo, collapse = " ")
...
Finally, let's put our functions to the test! They will be compared, at n = 2, both to each other and official tokenizers found on CRAN.


```{r benchmark}

tokenizer_speeds <- bench::mark(
  ...
  `quanteda` = as.character(quanteda::tokens_ngrams(x = quanteda::tokens(cristo$text), n = 2)),
  
  
  # Returning a tibble
  `Heads & Tails Tibble` = f1_tibble(words = cristo$text),
  `Nested Loops Tibble` = f2_tibble(words = cristo$text, n = 2),
  `Shift List Tibble` = f3_tibble(words = cristo$text, n = 2),
  
  `Nested Loops Tibble Custom` = f2_tibble_custom(text_df = cristo, key_column = "doc_id", text_column = "text", n = 2),
